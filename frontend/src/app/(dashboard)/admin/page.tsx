...
import { Loader2, Users, CreditCard, Activity, AlertTriangle, Dumbbell, Bell, DollarSign, TrendingUp } from "lucide-react";
...
export default function AdminPage() {
    ...
    return (
        <div className="space-y-6 p-6 animate-fade-in-up">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 animate-fade-in-up delay-150">
                <Card className="glass-card shadow-lg shadow-emerald-900/10 lg:col-span-2">
                    <CardHeader className="flex flex-row items-center justify-between pb-2">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold text-emerald-500">
                            R$ {stats?.revenue?.mrr?.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || '0,00'}
                        </div>
                        <p className="text-xs text-muted-foreground mt-1">
                            {stats?.subscriptions?.total_active || 0} assinaturas ativas
                        </p>
                    </CardContent>
                </Card>

                <Card className="glass-card shadow-lg shadow-gray-900/10">
                    <CardHeader className="flex flex-row items-center justify-between pb-2">
                        <CardTitle className="text-sm font-medium text-muted-foreground">Plano Free</CardTitle>
                        <Users className="h-4 w-4 text-gray-500" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold">{stats?.subscriptions?.free || 0}</div>
                        <p className="text-xs text-muted-foreground">usuários</p>
                    </CardContent>
                </Card>

                <Card className="glass-card shadow-lg shadow-amber-900/10">
                    <CardHeader className="flex flex-row items-center justify-between pb-2">
                        <CardTitle className="text-sm font-medium text-muted-foreground">Plano Premium</CardTitle>
                        <TrendingUp className="h-4 w-4 text-amber-500" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold">{stats?.subscriptions?.premium || 0}</div>
                ...
                <Card className="glass-card shadow-lg shadow-purple-900/10">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{stats?.subscriptions?.pro || 0}</div>
                        <p className="text-xs text-muted-foreground">assinantes</p>
                    </CardContent>
                </Card>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-fade-in-up delay-200">
                ...
                <Card className="glass-card cursor-pointer hover:bg-white/5 transition-all hover:scale-105 duration-300 group" onClick={() => router.push('/admin/audit-logs')}>
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2 group-hover:text-primary transition-colors">
                            <Activity className="h-5 w-5" />
                            Logs de Auditoria
                    ...
                    <CardContent>
                        <p className="text-sm text-muted-foreground">
                            Histórico completo de ações administrativas e mudanças do sistema.
                        </p>
                    </CardContent>
                </Card>
            </div>
